    ...
    <header className="fixed top-0 left-0 right-0 z-50">
      <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 py-3">
        <div className="flex items-center gap-3">
          {/* 글래스 네비 박스 */}
          <nav className="glass rounded-2xl px-3 sm:px-4 lg:px-5 py-2.5 flex-1 min-w-0">
            <div className="flex items-center gap-2">
              ...
              {/* Desktop Navigation */}
              <div className="hidden lg:flex items-center gap-0.5 xl:gap-1 ml-2">
                {NAV_LINKS.map((link) => (
                  <Link
                    key={link.href}
                    href={link.href}
                    className={`px-2 xl:px-2.5 py-1.5 text-[13px] font-medium rounded-lg transition-all flex items-center gap-1 whitespace-nowrap ${
                      isActive(link.href)
              ...
              <button
                ...
                className="lg:hidden ml-auto p-2 text-dark-300 hover:text-white hover:bg-dark-800/60 rounded-xl transition-all"
                aria-label="메뉴 열기"
              >
                {isMobileMenuOpen ? (
                  <X className="w-5 h-5" />
