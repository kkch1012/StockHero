...
async function getStockQuote(symbol: string): Promise<{
  ...
  if (useKISAPI) {
    try {
      ...
      console.warn('KIS API failed, falling back to Naver:', error);
    }
  }
  ...
  const quote = await provider.getQuote(symbol);
  return {
    source: useKISAPI ? 'naver_fallback' : 'naver',
    data: {
      symbol: quote.symbol,
      name: quote.name,
      price: quote.price,
...
async function getMultipleStockQuotes(symbols: string[]): Promise<{
  ...
  if (useKISAPI) {
    try {
      ...
      // KIS에서 가져온 데이터가 있으면 반환
      if (Object.keys(data).length > 0) {
        return { source: 'kis', data };
      }
    } catch (error) {
      console.warn('KIS API failed for multiple symbols, falling back to Naver:', error);
    }
  }
  ...
  return { source: useKISAPI ? 'naver_fallback' : 'naver', data };
}
